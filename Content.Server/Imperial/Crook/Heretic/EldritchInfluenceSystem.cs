...
public sealed class EldritchInfluenceSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<HereticComponent, ComponentInit>(OnHereticInit);
    }

    #region Visibility Management
    private void OnHereticInit(EntityUid uid, HereticComponent comp, ComponentInit args)
    {
        if (!TryComp<EyeComponent>(uid, out var eye))
            return;
        _eye.SetVisibilityMask(uid, eye.VisibilityMask | (int)VisibilityFlags.EldritchInfluence);
    }
    #endregion
